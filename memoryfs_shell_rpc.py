...
class FSShell():
  ...
  def ls(self):
    ...
    while block_index <= (inobj.inode.size // BLOCK_SIZE):
      block = self.FileObject.RawBlocks.Get(
        inobj.inode.block_numbers[block_index])
      ...
      while current_position < end_position:
        ...
        entryinode = block[current_position + MAX_FILENAME:current_position +
  ...
  def showblock(self, n):
    ...
    if n < 0 or n >= TOTAL_NUM_BLOCKS:
      print('Error: block number ' + str(n) + ' not in valid range [0, ' +
            str(TOTAL_NUM_BLOCKS - 1) + ']')
      return -1
    logging.info('Block (strings in block) [' + str(n) + '] : \n' +
                 str((self.FileObject.RawBlocks.Get(n).decode(
...
# implements showblockslice (log slice of block n contents)
  ...
  def showblockslice(self, n, start, end):
    ...
    if start < 0 or start >= BLOCK_SIZE:
      print('Error: start ' + str(start) + 'not in valid range [0, ' +
            str(BLOCK_SIZE - 1) + ']')
      return -1
    if end < 0 or end >= BLOCK_SIZE or end <= start:
      print('Error: end ' + str(end) + 'not in valid range [0, ' +
            str(BLOCK_SIZE - 1) + ']')
      return -1
  ...
  def showinode(self, i):
    ...
    if i < 0 or i >= MAX_NUM_INODES:
      print('Error: inode number ' + str(i) + ' not in valid range [0, ' +
            str(MAX_NUM_INODES - 1) + ']')
      return -1
  ...
  def append(self, filename, string):
    ...
    if inobj.inode.type != INODE_TYPE_FILE:
      print("Error: not a file\n")
      return -1
    written, errorcode = self.FileObject.Write(i, inobj.inode.size,
  ...
  # implements repair procudure in client
  def repair(self, server_id):
    i, errorcode = self.FileObject.repair(server_id)
    if i != 0:
      print("Error: " + errorcode)
      return -1
    return 0
...
if __name__ == "__main__":
  ...
  logging.basicConfig(filename='memoryfs.log',
                      filemode='w',
                      level=logging.DEBUG)
