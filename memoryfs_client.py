...
class DiskBlocks():
  ...
  def __init__(self, args):
    ...
    if args.startport:
      ...
      print('Must specify starting port number')
      quit()

    # 
    #if args.repair:

    # START OF NEW CODE FOR SERVER OBJECT LIST
    #print(self.ns)
    self.block_server = []
    for i in range(self.ns):
      server_url = 'http://' + SERVER_ADDRESS + ':' + str(STARTPORT + i)
      self.block_server.append(
        xmlrpc.client.ServerProxy(server_url, use_builtin_types=True))
    # END OF NEW CODE

    # START OF OLD CODE
    # server_url = 'http://' + SERVER_ADDRESS + ':' + str(STARTPORT)
...
#HandleFSConstants: Modify the FS constants if provided in command line argument
  ...
  def HandleFSConstants(self, args):
    ...
    # Data blocks start at INODE_BLOCK_OFFSET + INODE_NUM_BLOCKS
    DATA_BLOCKS_OFFSET = INODE_BLOCK_OFFSET + INODE_NUM_BLOCKS
    #print ("Data block offset = ", DATA_BLOCKS_OFFSET)
  ...
  def byte_xor(self, a, b):
    c = []
    for i in range(BLOCK_SIZE):
      c.append(a[i] ^ b[i])
    return c
    
  # RAIDPut writes block_data to block_number of a single server, for RAID 1
  def RAIDPut(self, block_number, putdata, server_number):
    try:
      ret = self.block_server[server_number].Put(block_number, putdata)
      #print ("RAIDPut: physical_block_number: " + str(block_number) + " server number: " + str(server_number))
      return ret
      ...
      pass
      # logging.error("request was recieved")
    # if ret == -1:
    #   logging.error('Put: Server returns error within RAIDPut')
    #   quit()

  # Perform one server get, given server number
  def RAIDGet(self, block_number, server_number):
    try:
      data, error = self.block_server[server_number].Get(block_number)
    except ConnectionRefusedError:
      print("SERVER_DISCONNECTED GET " + str(server_number))
      data = -3
      #raise ConnectionRefusedError  #I dunno what this does ahhhhhh
    except socket.timeout:
      print("SERVER_TIMED_OUT")
    else:
      pass
      # logging.error('Request was received')
    # check for corrupt block errors
    if data == -1:
      print("CORRUPTED_BLOCK " + str(block_number))

    if data == -2:  # bear with me checking for checksum error
      print(error)

    return data  #should we return as bytearray(data)
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  def Put(self, block_number, block_data):
    ...
    #logging.debug('Put: block number ' + str(block_number) + ' len ' +
                 # str(len(block_data)) + '\n' + str(block_data.hex()))
    if len(block_data) > BLOCK_SIZE:
      logging.error('Put: Block larger than BLOCK_SIZE: ' +
                    str(len(block_data)))
      quit()

    if block_number in range(0, TOTAL_NUM_BLOCKS):
      # ljust does the padding with zeros
      new_data = block_data.ljust(BLOCK_SIZE, b'\x00')
      putdata = bytearray(block_data.ljust(BLOCK_SIZE, b'\x00'))

      # David RAID4/5 draft below
      # RAID4 data block mapping:
      # server to write data to is block_number % ns
      # server's physical block number is block_number / ns (integer division)

      physical_block_number = block_number // (self.ns - 1)
      server_number = block_number % (self.ns - 1)
      
      # RAID4 parity
      # parity server is the last server
      # RAID5 data block mapping and parity server is computed here:
      parity_server = (self.ns - 1) - (block_number // (self.ns - 1)) % self.ns
      # shift data server to the right if parity server takes the data server or one to the left
      if parity_server <= server_number:
        server_number += 1
      # recompute parity by xoring new data and other servers' data,
      # or if one server is unavailable: xoring new data, other servers' data, and old parity
      #print("PUT: Virtual block number: " + str(block_number) + " Physical block number: " + str(physical_block_number) + " Server number: " + str(server_number))
      # store list of old block's data, including old parity
      # however, if data is
      old_data = []
      server_failure = None
      for i in range(self.ns):
        raw_data = self.RAIDGet(physical_block_number, i)
        # check if any RAIDGets fail and indicate a downed server - if so, store it
        # RAIDGet returns an int if there's an error
        if type(raw_data) == int:
          if raw_data < 0:
            old_data.append(raw_data)
            server_failure = i
            #print ("PUT Failure, server number:", server_failure)
        else:
          old_data.append(bytearray(raw_data))

      ##### RAID5 PARITY WRITE START #####
      # need list of servers that ARE NOT being written to
      # compute parity according to server availability
      # if no servers are disconnected and no blocks are corrupted
      # or, the failed server is the data server
      if server_failure == parity_server or server_failure == server_number:
        #print ("Put xoring new data with data from servers:")
        new_parity = new_data
        for i in range(self.ns): 
          # xor new data with existing data from other data servers ONLY
          # if not (i == server_number or i == parity_server):
          if (i != server_number) and (i != parity_server):
            #print ("Put server number: " + str(server_number) + " parity_server: " + str(parity_server))
            #print (i)
            #new_parity = self.byte_xor(new_parity, old_data[i])
            for k in range(BLOCK_SIZE):
              new_parity[k] = new_parity[k] ^ old_data[i][k]
              
        # write new parity, physical block number should be the same
        ret = self.RAIDPut(physical_block_number, new_parity, parity_server)
        
      # else, there's no server failure OR other data server failure
      # xor new data with new data block, old data block, and old parity
      else:
        #new_parity = self.byte_xor(new_data, old_data[server_number])
        #new_parity = self.byte_xor(new_parity, old_data[self.ns - 1])
        new_parity = new_data
        for k in range(BLOCK_SIZE):
          new_parity[k] = new_parity[k] ^ old_data[server_number][k] ^ old_data[parity_server][k]
          
        # ret = self.RAIDPut(physical_block_number, new_parity, self.ns - 1)
        ret = self.RAIDPut(physical_block_number, new_parity, parity_server)

      ##### RAID5 PARITY WRITE END

      
      # write the actual fucking data
      # if the data server to be written to is disconnected or returns an error, that's ok
      # parity was still updated so data can be reconstructed
      ret = self.RAIDPut(physical_block_number, putdata, server_number)
      
      # # check for corrupt block errors
      # if ret == -1:
      #   print(
      #     "CORRUPTED_BLOCK " +
      #     str(block_number))  #this is going to print twice idk if we want that
      # if ret == -2:  # bear with me checking for checksum error, this is also a duplicate
      #   print("CHECKSUM ERROR" + str(block_number))
      #   # need to add logic to connect to other servers and build from different servers upon failure
      #   # consider Get() calling a different function that does the above logic
  ...
  #     # ACCESSING LIST, WRITE TO ALL SERVERS
  #     # Implement at most once semantics
  #     for server_number in range(self.ns):
  #       ret = self.RAIDPut(block_number, putdata, server_number)
  #       # check for corrupt block errors
  #       if ret == -1:
  #         print("CORRUPTED_BLOCK " + str(block_number)
  #               )  #this is going to print twice idk if we want that
  #       if ret == -2:  # bear with me checking for checksum error, this is also a duplicate
  #         print("CHECKSUM ERROR" + str(block_number))
  #         # need to add logic to connect to other servers and build from different servers upon failure
  #         # consider Get() calling a different function that does the above logic

  #     # cams draft at multiple servers for RAID1
  #     # while (ret == -1) or (ret == -2):
  #     #   for i in range(self.ns):
  #     #     ret = self.RAIDPut(block_number, putdata, i)
  #       if ret == -1:
  #         logging.error('Put: Server ' + str(server_number) + ' returns error')

  #     # Put() shouldn't have any corrupt block errors, as data is being written to server and blocks on only corrupted via server
  #     return 0
  #   else:
  #     logging.error('Put: Block out of range: ' + str(block_number))
  #     quit()
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def Get(self, block_number):
    ...
    if block_number in range(0, TOTAL_NUM_BLOCKS):
      ...
      # David RAID4/5 draft
      # as in Put(), compute server and physical block number
      physical_block_number = block_number // (self.ns - 1)
      server_number = block_number % (self.ns - 1)
      ...
      #print("GET: Virtual block number: " + str(block_number) + " Physical block number: " + str(physical_block_number) + " Server number: " + str(server_number))
      
      data = self.RAIDGet(physical_block_number, server_number)
      # #print(data)
      # # check for corrupt block errors
      # if data == -1:
      #   print(
      #     "CORRUPTED_BLOCK " +
      #     str(block_number))  #this is going to print twice idk if we want that
      # if data == -2:  # bear with me checking for checksum error, this is also a duplicate
      #   print("CHECKSUM ERROR " + str(block_number))
      # if data == -3:
      #   print ("GET Failure, server number:", server_number)
      
      # we need to cleanup these error codes haha

      if data == -1 or data == -2 or data == -3:
        
        old_data = []
        # access all data blocks and parity that aren't corrupted

        
        #print("Xoring data from server 0 with: ")
        for i in range(self.ns):
          if i != server_number:
            old_data.append(bytearray(self.RAIDGet(physical_block_number, i)))
            #print(i)

        # get first block, then xor it with remaining blocks
        data = old_data[0]
        for i in range(1, len(old_data)): 
          for k in range(BLOCK_SIZE):
            data[k] = data[k] ^ old_data[i][k]  
          
          #data = self.byte_xor(data, old_data[i])
      # return as bytearray

      return bytearray(data)

    logging.error('Get: Block number larger than TOTAL_NUM_BLOCKS: ' +
                  str(block_number))
    quit()
  ...
  #     # ACCESSING LIST, FOR RAID1, get from server 0
  #     # to expand RAID1, need to try server 0 first, if checksum error (at-most-once), or server disconnected,

  #     data = self.RAIDGet(block_number, 0)
  #     # check for corrupt block errors
  #     if data == -1:
  #       print("CORRUPTED_BLOCK " + str(block_number))  #this is going to print twice idk if we want that
  #     if data == -2:  # bear with me checking for checksum error, this is also a duplicate
  #       print("CHECKSUM ERROR" + str(block_number))
  #       # need to add logic to connect to other servers and build from different servers upon failure
  #       # consider Get() calling a different function that does the above logic
  ...
  #     if data == -1 or data == -2 or data == -3:
  #       for i in range(self.ns):
  #         data = self.RAIDGet(block_number, i)
  #         print("hitting server: " + str(i))
...
## RSM: read and set memory equivalent
  ...
  def RSM(self, block_number):
    ...
    if block_number in range(0, TOTAL_NUM_BLOCKS):
      # ACCESSING LIST, RSM should only talk to server 0
    ...
    logging.error('RSM: Block number larger than TOTAL_NUM_BLOCKS: ' +
                  str(block_number))
    quit()
...
## Acquire and Release using a disk block lock
  ...
  def Acquire(self):
    ...
    while lockvalue[
        0] == 1:  # test just first byte of block to check if RSM_LOCKED
  ...
  def LoadFromDisk(self, filename):
    ...
    try:
      ...
      if file_system_constants != read_file_system_constants:
        print('Error: File System constants of File :' +
...
## This is used when we read an inode from raw block storage to the inode data structure
  ...
  def InodeFromBytearray(self, b):
    ...
    for i in range(0, MAX_INODE_BLOCK_NUMBERS):
      ...
      self.block_numbers[i] = int.from_bytes(blocknumber_slice,
                                             byteorder='big')
...
## The inode data structure loaded from raw storage goes in the self.inode object
  ...
  def InodeNumberToInode(self):
    ...
    raw_block_number = INODE_BLOCK_OFFSET + (
      (self.inode_number * INODE_SIZE) // BLOCK_SIZE)
    ...
    logging.debug('InodeNumberToInode : inode_number ' +
                  str(self.inode_number) + ' raw_block_number: ' +
                  str(raw_block_number) + ' slice start: ' + str(start) +
                  ' end: ' + str(end))
...
## The index selects which file name entry to extract within the block - e.g. index 0 is the first file name, 1 second file name
  ...
  def HelperGetFilenameString(self, block, index):
    ...
    logging.debug('HelperGetFilenameString: ' + str(block.hex()) + ', ' +
                  str(index))
...
## The index selects which entry to extract within the block - e.g. index 0 is the inode for the first file name, 1 second file name
  ...
  def HelperGetFilenameInodeNumber(self, block, index):
    ...
    logging.debug('HelperGetFilenameInodeNumber: ' + str(block.hex()) + ', ' +
                  str(index))
...
## insert_into is an InodeNumber() object; filename is a string; inodenumber is an integer
  ...
  def InsertFilenameInodeNumber(self, insert_to, filename, inodenumber):
    ...
    logging.debug('InsertFilenameInodeNumber: ' + str(filename) + ', ' +
                  str(inodenumber))
    ...
    if insert_to.inode.type != INODE_TYPE_DIR:
      logging.error('InsertFilenameInodeNumber: not a directory inode: ' +
                    str(insert_to.inode.type))
      quit()
    ...
    if index >= MAX_FILE_SIZE:
      logging.error(
        'InsertFilenameInodeNumber: no space for another entry in inode')
      quit()
    ...
    logging.debug('InsertFilenameInodeNumber: inode_start ' +
                  str(inode_start) + ', inode_end ' + str(inode_end))
    logging.debug('InsertFilenameInodeNumber: string_start ' +
                  str(string_start) + ', string_end ' + str(string_end))

    # Update and write data block with (filename,inode) mapping
    block[inode_start:inode_end] = inodenumber.to_bytes(
      INODE_NUMBER_DIRENTRY_SIZE, 'big')
    block[string_start:string_end] = bytearray(
      stringbyte.ljust(MAX_FILENAME, b'\x00'))
...
## Lookup string filename in the context of inode dir - same as textbook's LOOKUP
  ...
  def Lookup(self, filename, dir):
    ...
    if inode_number.inode.type != INODE_TYPE_DIR:
      logging.error("Lookup: not a directory inode: " + str(dir) + " , " +
                    str(inode_number.inode.type))
      return -1
    ...
    while offset < inode_number.inode.size:
      ...
      for i in range(0, FILE_ENTRIES_PER_DATA_BLOCK):
        ...
        if inode_number.inode.size > scanned:
          ...
          logging.debug("Lookup for " + filename + " in " + str(dir) +
                        ": searching string " + str(filestring))

          # Pad filename with zeroes and make it a byte array
          padded_filename = bytearray(filename, "utf-8")
          padded_filename = bytearray(
            padded_filename.ljust(MAX_FILENAME, b'\x00'))
    ...
    logging.debug("Lookup: file not found: " + str(filename) + " in " +
                  str(dir))
    return -1
...
## name is the object's name
  ...
  def Create(self, dir, name, type):
    ...
    logging.debug("Create: dir: " + str(dir) + ", name: " + str(name) +
                  ", type: " + str(type))
    ...
    logging.debug("Create: inode_position: " + str(inode_position) +
                  ", fileentry_position: " + str(fileentry_position))
...
## returns number of bytes written
  ...
  def Write(self, file_inode_number, offset, data):
    ...
    logging.debug("Write: file_inode_number: " + str(file_inode_number) +
                  ", offset: " + str(offset) + ", len(data): " +
                  str(len(data)))
    ...
    while bytes_written < len(data):
      ...
      logging.debug('Write: current_block_index: ' + str(current_block_index) +
                    ' , next_block_boundary: ' + str(next_block_boundary))
      ...
      logging.debug('Write: write_start: ' + str(write_start) +
                    ' , write_end: ' + str(write_end))
      ...
      block[write_start:write_end] = data[bytes_written:bytes_written +
      ...
      logging.debug('Write: current_offset: ' + str(current_offset) +
  ...
  def Read(self, file_inode_number, offset, count):
    ...
    logging.debug("Read: file_inode_number: " + str(file_inode_number) +
    ...
    while bytes_read < bytes_to_read:
      ...
      logging.debug('Read: current_block_index: ' + str(current_block_index) +
                    ' , next_block_boundary: ' + str(next_block_boundary))
      ...
      logging.debug('Read: read_start: ' + str(read_start) + ' , read_end: ' +
                    str(read_end))
      ...
      # copy slice of data into the right position in the block
      read_block[bytes_read:bytes_read +
                 (read_end - read_start)] = block[read_start:read_end]

      bytes_read += read_end - read_start
      current_offset += read_end - read_start

      logging.debug('Read: current_offset: ' + str(current_offset) +
    ...
    while block_index <= (dir_inode.inode.size // BLOCK_SIZE):
      ...
      while current_position < end_position:
        ...
        if entryname_padded == padded_filename:
          ...
          last_block = self.RawBlocks.Get(
            dir_inode.inode.block_numbers[last_block_index])
          last_block_end_position = (dir_inode.inode.size -
                                     FILE_NAME_DIRENTRY_SIZE) % BLOCK_SIZE
          if (last_block_index == block_index) and (last_block_end_position
                                                    == current_position):
            ...
            block[current_position:current_position +
                  FILE_NAME_DIRENTRY_SIZE] = bytearray(FILE_NAME_DIRENTRY_SIZE)
          else:
            ...
            # copy from last
            block[current_position:current_position +
                  FILE_NAME_DIRENTRY_SIZE] = last_block[
                    last_block_end_position:last_block_end_position +
                    FILE_NAME_DIRENTRY_SIZE]
            last_block[last_block_end_position:last_block_end_position +
                       FILE_NAME_DIRENTRY_SIZE] = bytearray(
                         FILE_NAME_DIRENTRY_SIZE)
...
# Resolve a relative path to an inode number
  ...
  def PathToInodeNumber(self, path, dir):
    ...
    logging.debug("PathToInodeNumber: path: " + str(path) + ", dir: " +
                  str(dir))
    ...
    if "/" in path:
      ...
      logging.debug("PathToInodeNumber: first: " + str(first) + ", rest: " +
                    str(rest))
...
# Resolve relative and absolute paths to an inode number
  ...
  def GeneralPathToInodeNumber(self, path, cwd):
    ...
    logging.debug("GeneralPathToInodeNumber: path: " + str(path) + ", cwd: " +
                  str(cwd))
  ...
  def GeneralPathToInodeNumber_Soft(self, path, cwd):
    ...
    logging.debug("GeneralPathToInodeNumber_Soft: path: " + str(path) +
                  ", cwd: " + str(cwd))
  ...
  def Link(self, target, name, cwd):
    ...
    logging.debug("Link: target: " + str(target) + ", name: " + str(name) +
                  ", cwd: " + str(cwd))
    ...
    target_inode_number_object = InodeNumber(self.RawBlocks,
                                             target_inode_number)
  ...
  def Symlink(self, target, name, cwd):
    ...
    logging.debug("Symlink: target: " + str(target) + ", name: " + str(name) +
                  ", cwd: " + str(cwd))
  ...
  def repair(self, server_id):
    ...
    # reconnects to server id
    #self.block_server.Get(server_id)
    # regenerates blocks for server id using data from the other servers in the array 
    #self.block_server[server_id].DiskBlocks(TOTAL_NUM_BLOCKS, BLOCK_SIZE)
    # server_num = int(server_id)
    # old_data = []
    # # access all data blocks and parity that aren't corrupted
    # for j in range(TOTAL_NUM_BLOCKS):
    #   for i in range(self.RawBlocks.ns):
    #     if i != server_num:
    #       old_data.append(bytearray(self.RawBlocks.Get(j)))
    #   # get first block, then xor it with remaining blocks
      
    # for i in range(1, len(old_data)):
    #   data = old_data[i]
    #   #data = self.RawBlocks.byte_xor(data, old_data[i])
    #   physical_block_number = j // (self.RawBlocks.ns - 1)
    #   for k in range(BLOCK_SIZE):
    #     data[k] = data[k] ^ old_data[i][k]
          
    #   self.RawBlocks.RAIDPut(physical_block_number, data, server_num)

    # get server_number of server to be repaired
    server_num = int(server_id)
    # loop through each physical block
    for i in range (TOTAL_NUM_BLOCKS//(self.RawBlocks.ns - 1)):
      old_data = []
      # get data of physical block i from server j
      for j in range (self.RawBlocks.ns):
        if j != server_num:
          old_data.append(bytearray(self.RawBlocks.RAIDGet(i, j)))

      data = old_data[0]
      for j in range (1, self.RawBlocks.ns - 1):
        for k in range(BLOCK_SIZE):
          data[k] = data[k] ^ old_data[j][k]
      
      self.RawBlocks.RAIDPut(i, data, server_num)
